...
from elysia.config import (
    ...
    load_base_lm,
    load_complex_lm,
    settings as environment_settings,
)
...
from elysia.tools.courthouse.courthouse_debate import CourthouseDebate
...
from elysia.tree.objects import Atlas, CollectionData, Environment, TreeData
...
class Tree:
    ...
    @property
    def base_lm(self) -> "dspy.LM | GPT5ResponsesClient":
        if self.low_memory:
    ...
    @property
    def complex_lm(self) -> "dspy.LM | GPT5ResponsesClient":
        if self.low_memory:
    ...
    def multi_branch_init(self) -> None:
        # Add domain routing as the root branch
        self.add_branch(
            root=True,
            branch_id="domain_routing",
            instruction="""
            First, evaluate if this query requires specialized domain knowledge.
            Use the domain router to classify the query and route to the appropriate custom agent if needed.
            """,
            status="Evaluating query domain...",
        )
        self.add_tool(branch_id="domain_routing", tool=DomainRouter)

        # Add base branch for general processing
        self.add_branch(
            root=False,
            branch_id="base",
            from_branch_id="domain_routing",
            instruction="""
            ...
            """,
            description="Handle general queries that don't require specialized legal domain knowledge.",
    ...
    def one_branch_init(self) -> None:
        self.add_branch(
            ...
            instruction="""
            Choose a base-level task based on the user's prompt and available information.
            First, use the domain router to check if this requires specialized domain knowledge.
            If it does, the appropriate custom agent will handle it.
            Otherwise, decide based on the tools you have available as well as their descriptions.
            Read them thoroughly and match the actions to the user prompt.
            """,
            status="Choosing a base-level task...",
        )
        # Add domain router first
        self.add_tool(branch_id="base", tool=DomainRouter)
    ...
    def soft_reset(self) -> None:
        ...
        # environment is not reset (preserves retrieved objects across queries)
        # BUT hidden_environment IS reset (inter-tool state should not persist)
        if self.low_memory:
            self.history = {}

        # Clear hidden_environment to prevent inter-tool state from persisting across queries
        # This fixes the issue where stale state from previous queries causes decision tree loops
        self.tree_data.environment.hidden_environment = {}
        ...
        # Reset error counters for circuit breaker
    ...
    async def _evaluate_result(
        ...
        if isinstance(result, Text):
            # If courthouse mode is enabled and this is the initial response, capture it
            if (
                hasattr(self, "_is_courthouse_mode")
                and self._is_courthouse_mode
                and self._courthouse_initial_response is None
            ):
                self.settings.logger.info(
                    "Courthouse Mode: Capturing initial response for debate"
                )
                self._courthouse_initial_response = result.text
                # Extract sources from environment
                self._courthouse_initial_sources = []
                for tool_name in self.tree_data.environment.environment:
                    for result_name in self.tree_data.environment.environment[
                        tool_name
                    ]:
                        objects = self.tree_data.environment.environment[tool_name][
                            result_name
                        ]
                        self._courthouse_initial_sources.extend(objects)
                # Don't update conversation history or print yet - will be done after debate
                return None, error
            # If intelligence mode is enabled and this is the initial response, capture it
            elif (
                hasattr(self, "_is_intelligence_mode")
                and self._is_intelligence_mode
                and self._intelligence_initial_response is None
            ):
                self.settings.logger.info(
                    "Intelligence Mode: Capturing initial response for analysis"
                )
                self._intelligence_initial_response = result.text
                # Extract sources from environment
                self._intelligence_initial_sources = []
                for tool_name in self.tree_data.environment.environment:
                    for result_name in self.tree_data.environment.environment[
                        tool_name
                    ]:
                        ...
                        self._intelligence_initial_sources.extend(objects)
                # Don't update conversation history or print yet - will be done after analysis
                return None, error
            else:
    ...
    async def async_run(
        ...
        is_courthouse_mode: bool = False,
        is_intelligence_mode: bool = False,
        **kwargs,
        ...
        if _first_run:
            # Store courthouse mode flag for this query
            self._is_courthouse_mode = is_courthouse_mode
            self._courthouse_initial_response = None
            self._courthouse_initial_sources = []

            # Store intelligence mode flag for this query
            self._is_intelligence_mode = is_intelligence_mode
            self._intelligence_initial_response = None
            self._intelligence_initial_sources = []
            ...
            if is_courthouse_mode:
                self.settings.logger.info("Courthouse Mode enabled for this query")

            if is_intelligence_mode:
                self.settings.logger.info("Intelligence Mode enabled for this query")
            ...
            self.soft_reset()
            self.settings.logger.debug(
                f"Soft reset completed for query {query_id}. Hidden environment cleared."
            )
            ...
            # Store custom agent metadata in hidden_environment for router access
            # Note: Custom agents are loaded during tree initialization, so we just need to
            # set the metadata here (hidden_environment is cleared by soft_reset)
            if self.user_id and client_manager.is_client:
                try:
                    agent_registry = CustomAgentRegistry(logger=self.settings.logger)
                    # Only load agent metadata, tools are already added during initialization
                    await agent_registry.load_custom_agents(
                        user_id=self.user_id,
                        client_manager=client_manager,
                    )

                    if len(agent_registry.get_agent_names()) > 0:
                        # Store custom agent metadata in hidden_environment for router access
                        self.tree_data.environment.hidden_environment[
                            "custom_agents"
                        ] = {
                        ...
                        self.settings.logger.debug(
                            f"Set custom agent metadata in hidden_environment for {len(agent_registry.get_agent_names())} agent(s)"
                        )
                except Exception as e:
                    self.settings.logger.warning(
                        f"Failed to set custom agent metadata for user {self.user_id}: {str(e)}"
                    )
        ...
        while True:
            ...
            if action_fn is not None:
                ...
                with ElysiaKeyManager(self.settings):
                    ...
                    tool_kwargs = {**kwargs, "user_id": self.user_id}
                ...
                if not successful_action:
                    ...
                    if self.consecutive_errors >= self.max_consecutive_errors:
                        self.settings.logger.error(
                            ...
                            "Stopping to prevent infinite loop and rate limiting."
                        )
                        completed = True  # Force completion to stop the loop
                    else:
                        completed = (
                            False
        ...
        if completed:
            ...
            # If courthouse mode is enabled and we captured an initial response, run the debate
            if (
                hasattr(self, "_is_courthouse_mode")
                and self._is_courthouse_mode
                and self._courthouse_initial_response is not None
            ):
                self.settings.logger.info(
                    "Courthouse Mode: Starting debate with captured response"
                )

                # Create courthouse debate tool instance
                courthouse_debate = CourthouseDebate(logger=self.settings.logger)

                # Create a fake decision for the courthouse debate
                courthouse_decision = Decision(
                    function_name="courthouse_debate",
                    function_inputs={
                        "initial_response": self._courthouse_initial_response,
                        "initial_sources": self._courthouse_initial_sources,
                        "initial_query": user_prompt,
                    },
                    reasoning="Running courthouse debate to refine response",
                    end_actions=False,
                    impossible=False,
                )

                # Run the courthouse debate
                with ElysiaKeyManager(self.settings):
                    async for result in courthouse_debate(
                        tree_data=self.tree_data,
                        inputs=courthouse_decision.function_inputs,
                        base_lm=self.base_lm,
                        complex_lm=self.complex_lm,
                        client_manager=client_manager,
                    ):
                        # For courthouse debate results, we want to yield them directly
                        # and update conversation history with the final response
                        if isinstance(result, Text):
                            # This is the final consensus - update conversation history
                            self._update_conversation_history("assistant", result.text)
                            if self.settings.LOGGING_LEVEL_INT <= 20:
                                print(
                                    Panel.fit(
                                        result.text,
                                        title="Courthouse Consensus",
                                        border_style="green",
                                        padding=(1, 1),
                                    )
                                )
                        # Yield all results (agent messages, status, final text)
                        action_result, _ = await self._evaluate_result(
                            result, courthouse_decision
                        )
                        if action_result is not None:
                            yield action_result

                self.settings.logger.info("Courthouse Mode: Debate completed")

            # If intelligence mode is enabled and we captured an initial response, run the analysis
            if (
                hasattr(self, "_is_intelligence_mode")
                and self._is_intelligence_mode
                and self._intelligence_initial_response is not None
            ):
                self.settings.logger.info(
                    "Intelligence Mode: Starting analysis with captured response"
                )

                # Create intelligence orchestrator tool instance
                intelligence_orchestrator = IntelligenceOrchestrator(
                    logger=self.settings.logger
                )

                # Create a fake decision for the intelligence analysis
                intelligence_decision = Decision(
                    function_name="intelligence_orchestrator",
                    function_inputs={
                        "initial_response": self._intelligence_initial_response,
                        "initial_sources": self._intelligence_initial_sources,
                        "initial_query": user_prompt,
                    },
                    reasoning="Running intelligence analysis to explore relationships",
                    end_actions=False,
                    impossible=False,
                )
                ...
                with ElysiaKeyManager(self.settings):
                    async for result in intelligence_orchestrator(
                        tree_data=self.tree_data,
                        inputs=intelligence_decision.function_inputs,
                        base_lm=self.base_lm,
                        complex_lm=self.complex_lm,
                        client_manager=client_manager,
                    ):
                        # For intelligence analysis results, we want to yield them directly
                        # and update conversation history with the final response
                        if isinstance(result, Text):
                            # This is the final analysis - update conversation history
                            self._update_conversation_history("assistant", result.text)
                            if self.settings.LOGGING_LEVEL_INT <= 20:
                                print(
                                    Panel.fit(
                        ...
                        # Yield all results (agent messages, charts, status, final text)
                        action_result, _ = await self._evaluate_result(
                            result, intelligence_decision
                        )
                        if action_result is not None:
                            yield action_result

                self.settings.logger.info("Intelligence Mode: Analysis completed")
