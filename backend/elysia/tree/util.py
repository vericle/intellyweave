...
from elysia.util.objects import FewShotExamples, TrainingUpdate, TreeUpdate
...
class DecisionNode:
    ...
    def _tool_assertion(self, kwargs, pred):
        # Check if function_name is in available actions
        if pred.function_name not in self.options:
            return (
                False,
        ...
        # Check for immediate repetition of the same action
        tree_data = kwargs.get("tree_data")
        if tree_data and tree_data.tasks_completed:
            # Get the last completed tasks for the current prompt
            if len(tree_data.tasks_completed) > 0:
                last_prompt_tasks = tree_data.tasks_completed[-1].get("task", [])

                # Check the last 2 tasks to detect immediate repetition
                if len(last_prompt_tasks) >= 1:
                    last_task = last_prompt_tasks[-1]
                    last_task_name = last_task.get("task", "")
                    last_task_error = last_task.get("error", False)

                    # If the last task was the same action and it succeeded, warn against repetition
                    if last_task_name == pred.function_name and not last_task_error:
                        # Check if inputs are meaningfully different
                        last_inputs = last_task.get("inputs", {})
                        current_inputs = pred.function_inputs

                        # If inputs are identical or very similar, reject the repetition
                        if last_inputs == current_inputs:
                            return (
                                False,
                                f"You just completed the action `{pred.function_name}` successfully with the same inputs. "
                                f"This would be a repetitive action. Review the environment and tasks_completed, then either:\n"
                                f"1. Choose a DIFFERENT action from available_actions, OR\n"
                                f"2. If you must use the same action, provide meaningfully different inputs that will produce different results, OR\n"
                                f"3. Set end_actions=True if you have sufficient information to answer the user's query.\n"
                                f"Available actions: {list(self.options.keys())}"
                            )
        ...
        if not one_choice:
            ...
            if tree_data.settings.USE_FEEDBACK:
                ...
                output, uuids = await decision_executor.aforward_with_feedback_examples(
                    ...
                    tree_data=tree_data,  # Pass tree_data for assertion
                )
            else:
