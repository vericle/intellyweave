...
class DecisionPrompt(dspy.Signature):
    ...
    function_name: str = dspy.OutputField(
        ...
        1. ALWAYS check previous_reasoning (tasks_completed) before selecting an action
        2. If an action was just completed successfully in the last 1-2 tasks, DO NOT select it again unless:
           - You have meaningfully different inputs that will produce different results
           - The user explicitly asks for more of the same type of information
        3. If you just retrieved documents/data successfully, DO NOT query the same collection again
        4. For simple queries that can be answered in 1-2 actions, prefer setting end_actions=True after the first successful retrieval

        ACTION SELECTION STRATEGY:
        - Use the action descriptions (and instruction, if given) to make your decision
        - Choose the action that will progress the user's request the most
        - Use `successive_actions` to inform your decision in how it will lead to future tools
        - If unsure but the action is related to the user prompt, try it even if it looks hard
        - You MAY select a function multiple times IF the inputs are meaningfully different
        - Avoid repetition: do not select the same action that just ran unless there is a clear, specific reason
    ...
    end_actions: bool = dspy.OutputField(
        ...
        Set end_actions=True when ANY of these conditions are met:
        1. The `end_goal` has been achieved with the selected action
        2. For simple queries: After successfully retrieving relevant data in 1-2 actions
        3. You have done everything possible with available actions
        4. The tree_count is at or near the recursion limit
        5. Further actions would be repetitive or not add value

        CRITICAL: Failing to set end_actions=True will result in continued responses and potential loops.
        For straightforward queries that can be answered with a single data retrieval, ALWAYS set end_actions=True after that retrieval.

        Set end_actions=False ONLY when:
        - The selected action is part of a multi-step process that requires subsequent actions
        - You need to retrieve from multiple collections to answer the query
        - The query explicitly requires aggregation or comparison across multiple data sources
        """.strip()
    )
