        ...
        for j, task_prompt in enumerate(self.tasks_completed):
            ...
            for i, task in enumerate(task_prompt["task"]):
                ...
                if "error" in task and task["error"]:
                    out += (
                        f"Status: FAILED - There was an error during this tool call. "
                        "See the error messages for details. This action did not complete.\n"
                    )
                else:
                    out += (
                        f"Status: COMPLETED SUCCESSFULLY - This action has finished executing. "
                        "The results are in the environment. Do NOT repeat this action unless inputs are meaningfully different.\n"
                    )
                    for key in task:
                        if key != "task" and key != "action" and key != "error" and key != "iteration":
