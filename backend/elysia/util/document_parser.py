...
class PDFParser:
    ...
    async def parse(self, file_path: Path) -> Dict[str, Any]:
        ...
        logger.debug("Starting async PDF parse for %s", file_path)
        result = await asyncio.get_event_loop().run_in_executor(
            self._executor, self._parse_sync, file_path
        )
        return result
...
class TextParser:
    ...
    async def parse(self, file_path: Path) -> Dict[str, Any]:
        """Parse text file"""
        try:
            logger.debug("Starting async text parse for %s", file_path)
            import aiofiles
...
class DocumentParserFactory:
    ...
    def get_parser(file_extension: str):
        ...
        logger.debug("Selecting document parser for extension %s", file_ext)
        ...
        logger.error("Unsupported document extension requested: %s", file_ext)
        raise ValueError(
