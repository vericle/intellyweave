...
async def retrieve_all_collection_names(client):
    ...
    # Prioritize chunk collections over document collections
    # Sort so ELYSIA_CHUNKED_* collections come before ELYSIA_UPLOADED_DOCUMENTS
    return sorted(filtered_collections, key=lambda x: (0 if x.startswith("ELYSIA_CHUNKED_") else 1, x))
