...
class ContextTruncator:
    ...
    def truncate_evidence_list(
        ...
        max_per_item_chars: int,
        content_key: str = "content",
        model: str = "gpt-5",
    ) -> list[dict]:
        """
        Truncate a list of evidence documents to fit within token limits.

        Args:
            evidence: List of evidence dicts with content
            max_total_tokens: Maximum total tokens for all evidence
            max_per_item_chars: Maximum characters per evidence item
            content_key: Key in dict containing content to truncate
            model: Model for token counting

        Returns:
            Truncated list of evidence dicts
        """
        if not evidence:
            return evidence
        ...
        for item in evidence:
            item_copy = item.copy()
            content = item_copy.get(content_key, "")

            # Truncate individual item content if too large
            if len(content) > max_per_item_chars:
                item_copy[content_key] = content[:max_per_item_chars] + "... [truncated]"

            item_tokens = self.token_counter.count_tokens(
                str(item_copy), model=model
            )

            if total_tokens + item_tokens > max_total_tokens:
                self.logger.info(
                    f"Truncated evidence list at {len(result)} items "
                    f"(would exceed {max_total_tokens} tokens)"
                )
                break
    ...
    def truncate_string_to_fit(
        ...
        """
        Truncate a string to fit within token limits when combined with other content.

        Used for pre-DSPy token checks to ensure combined inputs don't exceed
        model context limits. Truncates the main content while preserving other_content.

        Args:
            content: The main content string to potentially truncate
            other_content: Other content that will be combined (not truncated)
            max_tokens: Maximum total tokens allowed for content + other_content
            truncation_marker: String to append when truncation occurs
            model: Model for token counting
        ...
        total_tokens = self.token_counter.count_tokens(
            content + other_content, model=model
        )
        ...
        if len(content) > excess_chars:
            truncated_content = content[:-excess_chars] + truncation_marker
            self.logger.info(
                f"Truncated content by ~{excess_tokens // 1000}K tokens to fit "
                f"{max_tokens // 1000}K token limit"
            )
            return truncated_content, True

        # Content too small to truncate meaningfully
        self.logger.warning(
            f"Content ({len(content)} chars) too small to truncate by "
            f"{excess_chars} chars - returning as-is"
        )
        return content, False
