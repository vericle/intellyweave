    ...
    async def __call__(
        ...
        **kwargs,
        ...
        if "items_to_summarise" in tree_data.environment.hidden_environment:
            ...
            if self.logger:
                self.logger.debug(
                    f"Starting summarization of {len(objects_list)} item(s) from hidden_environment"
                )
            ...
            if self.logger:
                self.logger.debug(
                    "Completed summarization and cleared 'items_to_summarise' from hidden_environment"
                )
