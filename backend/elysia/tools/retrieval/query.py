...
class Query(Tool):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            name="query",
            description="""
            Retrieve documents containing specific entities for intelligence analysis.
            Use to find: all mentions of a person, organization activities in a location, events within a date range.
            Results include extracted entities (persons, organizations, locations, events, dates, laws) for relationship analysis.
            Supports semantic search, keyword search, or combination with entity-based filtering.
            Perfect for building intelligence profiles and discovering entity connections.
    ...
    def _sanitize_fields_to_search(self, fields_to_search: dict) -> dict:
        """
        Sanitize fields_to_search dictionary to handle LLM returning string "None" instead of Python None.
        Converts string "None" to Python None, and ensures values are either None or list[str].
        """
        if not fields_to_search:
            return {}

        sanitized = {}
        for collection_name, fields in fields_to_search.items():
            # Handle string "None" -> Python None
            if isinstance(fields, str) and fields.lower() in ("none", "null"):
                sanitized[collection_name] = None
            # Handle valid list
            elif isinstance(fields, list):
                sanitized[collection_name] = fields
            # Handle None (already correct)
            elif fields is None:
                sanitized[collection_name] = None
            else:
                # Fallback: convert to None if unrecognized type
                if self.logger:
                    self.logger.warning(
                        f"Unexpected type for fields_to_search[{collection_name}]: {type(fields)}. Converting to None."
                    )
                sanitized[collection_name] = None
    ...
    async def __call__(
        ...
        **kwargs,
        ...
        if self.logger:
            ...
            self.logger.debug(
                f"Hidden environment state: {list(tree_data.environment.hidden_environment.keys())}"
            )
        ...
        try:
            ...
            if self.logger:
                ...
                self.logger.error(f"This error occurred on query in conversation with {len(tree_data.conversation_history)} messages in history")
            yield Error(error_message=f"Query generation failed: {str(e)}")
            return

        # Sanitize fields_to_search to handle LLM returning string "None" instead of Python None
        if hasattr(query, 'fields_to_search') and query.fields_to_search is not None:
            query.fields_to_search = self._sanitize_fields_to_search(query.fields_to_search)
            if self.logger:
                self.logger.debug(f"Sanitized fields_to_search: {query.fields_to_search}")
        ...
        for i, query_output in enumerate(query.query_outputs):
            ...
            for k, (collection_name, response) in enumerate(
                ...
                if summarise_items and self.summariser_in_tree:
                    ...
                    if self.logger:
                        self.logger.debug(
                            f"Added items to hidden_environment for summarization. "
                            f"Total items pending: {len(tree_data.environment.hidden_environment['items_to_summarise'])}"
                        )
                else:
                    yield output
