...
class QueryCreatorPrompt(dspy.Signature):
    ...
    fields_to_search: dict[str, list[str] | None] = dspy.OutputField(
        ...
        These come from the `searchable_fields` list. If this list is empty, the value should be null (NOT None, use JSON null).
        Otherwise, you should choose the field(s) that would be most relevant to the user's query.
        If in doubt, set the value inside the dictionary to null (use proper JSON format, not Python None).
        CRITICAL: Use "null" for null values, not "None" (this must be valid JSON).
        """.strip()
    )
