...
class DataMappingPrompt(dspy.Signature):
    ...
    mapping_type: str = dspy.InputField(
        desc=(
            ...
            "Some examples are 'conversation', 'message', 'ticket', 'product', 'document', 'table', 'generic', 'mapbox', 'archives', 'investigation'. "
...
class PromptSuggestorPrompt(dspy.Signature):
    ...
    prompt_suggestions: list[str] = dspy.OutputField(
        desc="""
        Generate intelligence-focused questions for OSINT analysis.

        ENTITY-AWARE GENERATION:
        - Check for entity fields: persons, organizations, locations, events, dates, laws
        - Use ACTUAL entity names from the data when available
        - If no entities, use field values intelligently

        QUESTION PATTERNS (7-12 words, action-oriented):

        For PERSONS:
        - "Analyze [Person Name]'s connections and activities"
        - "Show network relationships for [Person Name]"
        - "Timeline of [Person Name]'s mentioned events"

        For ORGANIZATIONS:
        - "Map [Organization]'s geographic presence and operations"
        - "Analyze [Organization] personnel and connections"
        - "Show [Organization] activities by location"

        For LOCATIONS:
        - "Which entities operate in [Location]"
        - "Map all mentions of [Location]"
        - "Timeline of events in [Location]"

        For RELATIONSHIPS:
        - "Connection between [Entity A] and [Entity B]"
        - "Common locations for [Person] and [Organization]"
        - "Network overlap between [Org A] and [Org B]"

        For TEMPORAL:
        - "Events between [Date1] and [Date2]"
        - "Timeline of [Organization] activities in [Year]"
        - "Chronological progression of [Event type]"

        REQUIREMENTS:
        - Generate exactly 10 questions
        - Use action verbs: Analyze, Map, Show, Timeline, Compare
        - Include real entity names, not placeholders
        - Mix different pattern types
        - Focus on intelligence analysis, not generic queries
        """
    )
