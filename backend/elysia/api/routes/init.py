...
async def initialise_tree(
    ...
    try:
        ...
        # Load custom agents after tree initialization so they appear in the tree structure
        user = await user_manager.get_user_local(user_id)
        ...
        if client_manager.is_client:
            ...
            try:
                ...
                agent_registry = CustomAgentRegistry(logger=logger)
                custom_agents = await agent_registry.load_custom_agents(
                    user_id=user_id,
                    client_manager=client_manager,
                )

                if len(custom_agents) > 0:
                    # Determine which branch to add agents to based on branch initialisation
                    if tree.branch_initialisation == "multi_branch":
                        agent_branch = "domain_routing"
                    else:
                        agent_branch = "base"

                    for agent_tool in custom_agents:
                        try:
                            tree.add_tool(branch_id=agent_branch, tool=agent_tool)
                            logger.info(f"Loaded custom agent during tree init: {agent_tool.name}")
                        except Exception as tool_error:
                            logger.error(
                                f"Failed to add custom agent {agent_tool.name} during init: {str(tool_error)}"
                            )

                    logger.info(
                        f"Successfully loaded {len(custom_agents)} custom agent(s) during tree initialization for user {user_id}"
                    )
            except Exception as e:
                logger.warning(
                    f"Failed to load custom agents during tree initialization for user {user_id}: {str(e)}"
                )
                # Continue execution even if custom agent loading fails
