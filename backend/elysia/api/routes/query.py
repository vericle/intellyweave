...
from elysia.api.utils.websocket import help_websocket, safe_websocket_send
...
async def process(data: dict, websocket: WebSocket, user_manager: UserManager):
    ...
    logger.debug(f"Courthouse mode: {data.get('is_courthouse_mode', False)}")
    logger.debug(f"Intelligence mode: {data.get('is_intelligence_mode', False)}")
    ...
    try:
        ...
        async for yielded_result in user_manager.process_tree(
            ...
            is_courthouse_mode=data.get("is_courthouse_mode", False),
            is_intelligence_mode=data.get("is_intelligence_mode", False),
        ):
            ...
            try:
                if (
                    ...
                    await safe_websocket_send(websocket, yielded_result)
