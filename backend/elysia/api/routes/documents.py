...
async def upload_document(
    ...
    auto_geocode: bool = Form(True),
    create_agent: bool = Form(False),
    ...
    Optionally create a custom agent using the uploaded document as its knowledge base.
    ...
    Args:
        ...
        auto_geocode: Whether to automatically geocode extracted locations (default: True)
        create_agent: Whether to create a custom agent with this document (default: False)
        system_prompt: System instructions for the custom agent (required if create_agent=True)
        user_manager: The user manager dependency

    Returns:
        JSONResponse with upload results and optional agent metadata
    """
    ...
    try:
        ...
        # Validate custom agent parameters
        if create_agent and (not system_prompt or system_prompt.strip() == ""):
            raise HTTPException(
                status_code=400,
                detail="system_prompt is required when create_agent=True"
            )
        ...
        try:
            ...
            if result["success"]:
                # If create_agent=True, create custom agent with uploaded document
                if create_agent and system_prompt:
                    try:
                        agent_result = await document_service.create_custom_agent(
                        ...
                        logger.info(
                            f"Created custom agent '{agent_result['agent_name']}' "
                            f"for document {result['document_id']}"
                        )
                    except Exception as agent_error:
                        logger.error(f"Failed to create custom agent: {str(agent_error)}")
...
async def delete_document(
    ...
    Args:
        ...
        reprocess: Whether to re-preprocess the collection after deletion to update
                   metadata statistics. Set to False for batch deletions. Default: True
        user_manager: The user manager dependency

    Returns:
        JSONResponse with deletion result
    """
    logger.info(f"Delete document request: user={user_id}, document={document_id}, reprocess={reprocess}")
