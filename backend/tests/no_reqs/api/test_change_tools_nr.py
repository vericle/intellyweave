...
async def test_add_branch_to_branch():
    ...
    # check returned tree is correct
    # Updated path to include 'base' branch due to multi_branch root change
    assert "new_branch" in response["tree"]["options"]["base"]["options"]["search"]["options"]
    assert (
        response["tree"]["options"]["base"]["options"]["search"]["options"]["new_branch"]["id"]
        == "new_branch"
    )
    assert (
        response["tree"]["options"]["base"]["options"]["search"]["options"]["new_branch"]["description"]
        == "New Branch Description"
    )
    assert (
        response["tree"]["options"]["base"]["options"]["search"]["options"]["new_branch"]["instruction"]
        == "New Branch Instruction"
    )
    assert (
        response["tree"]["options"]["base"]["options"]["search"]["options"]["new_branch"]["branch"] is True
    )
    assert (
        response["tree"]["options"]["base"]["options"]["search"]["options"]["new_branch"]["options"] == {}
    )

    # check tree itself is correct
    tree = await user_manager.get_tree(user_id, conversation_id)
    assert "new_branch" in tree.tree["options"]["base"]["options"]["search"]["options"]
    assert (
        tree.tree["options"]["base"]["options"]["search"]["options"]["new_branch"]["id"]
        == "new_branch"
    )
    assert (
        tree.tree["options"]["base"]["options"]["search"]["options"]["new_branch"]["description"]
        == "New Branch Description"
    )
    assert (
        tree.tree["options"]["base"]["options"]["search"]["options"]["new_branch"]["instruction"]
        == "New Branch Instruction"
    )
    assert (
        tree.tree["options"]["base"]["options"]["search"]["options"]["new_branch"]["branch"] is True
    )
    assert (
        tree.tree["options"]["base"]["options"]["search"]["options"]["new_branch"]["options"] == {}
    )
...
async def test_full_cycle():
    ...
    assert (
        "new_branch"
        in response["tree"]["options"]["base"]["options"]["search"]["options"]["query"]["options"]
    )
    assert (
        response["tree"]["options"]["base"]["options"]["search"]["options"]["query"]["options"][
            "new_branch"
        ]["id"]
        == "new_branch"
    )
    ...
    tree = await user_manager.get_tree(user_id, conversation_id)
    assert "new_branch" in tree.tree["options"]["base"]["options"]["search"]["options"]["query"]["options"]
    assert (
        tree.tree["options"]["base"]["options"]["search"]["options"]["query"]["options"]["new_branch"][
            "id"
        ]
        == "new_branch"
    )
    ...
    assert (
        "tell_a_joke"
        in tree.tree["options"]["base"]["options"]["search"]["options"]["query"]["options"]["new_branch"][
            "options"
        ]
    )
    ...
    assert (
        "new_branch"
        not in response["tree"]["options"]["base"]["options"]["search"]["options"]["query"]["options"]
    )

    tree = await user_manager.get_tree(user_id, conversation_id)
    assert (
        "new_branch"
        not in tree.tree["options"]["base"]["options"]["search"]["options"]["query"]["options"]
    )
