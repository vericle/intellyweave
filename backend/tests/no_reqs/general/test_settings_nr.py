...
def test_basic_configure_local():
    ...
    """
    settings = Settings()
    # snapshot global values to ensure local configure does not mutate them
    orig_global_base_model = global_settings.BASE_MODEL
    orig_global_complex_model = global_settings.COMPLEX_MODEL
    orig_global_base_provider = global_settings.BASE_PROVIDER
    orig_global_complex_provider = global_settings.COMPLEX_PROVIDER

    # configure local complex model (anthropic provider always used)
    settings.configure(
    ...
    assert global_settings.COMPLEX_MODEL == orig_global_complex_model
    assert global_settings.COMPLEX_PROVIDER == orig_global_complex_provider

    # configure local base model (anthropic provider)
    settings.configure(base_model="claude-sonnet-4-5", base_provider="anthropic")
    assert settings.BASE_MODEL == "claude-sonnet-4-5"
    assert settings.BASE_PROVIDER == "anthropic"
    assert global_settings.BASE_MODEL == orig_global_base_model
    assert global_settings.BASE_PROVIDER == orig_global_base_provider

    # update base model again to a different anthropic model
    settings.configure(base_model="claude-haiku-4-5", base_provider="anthropic")
    assert settings.BASE_MODEL == "claude-haiku-4-5"
    assert settings.BASE_PROVIDER == "anthropic"
