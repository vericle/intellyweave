...
def test_data_analysis():   
    ...
    configure(
        base_model="claude-haiku-4-5",  # replace models and providers with which ever LM you want to use
    ...
    # First query to populate environment (regression tool requires this)
    tree(
        "Retrieve all data points from the diabetes dataset",
        collection_names=["ELYSIA_Test_Diabetes"],
    )
    
    # Then run regression on populated environment
    response, objects = tree(
