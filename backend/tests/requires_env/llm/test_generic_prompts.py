...
async def test_query():
    ...
    # Load missing datasets if not present
    ...
    if not client.collections.exists("Weather"):
        ...
        with client_manager.connect_to_client() as client_conn:
            ...
            weather_collection = client_conn.collections.get("Weather")
        ...
        with client_manager.connect_to_client() as client_conn:
            client_conn.collections.create(
                ...
                properties=[
                    ...
                    wvc.Property(name="price", data_type=wvc.DataType.NUMBER, description="price of item in USD"),
                ]
            )
            
            ecommerce_dataset = load_dataset("weaviate/agents", "query-agent-ecommerce", split="train", streaming=True)
            ecommerce_collection = client_conn.collections.get("Ecommerce")
    ...
    if not client.collections.exists("Brands"):
        with client_manager.connect_to_client() as client_conn:
            ...
            brands_collection = client_conn.collections.get("Brands")
            
            with brands_collection.batch.dynamic() as batch:
                for item in brands_dataset:
                    batch.add_object(properties=item["properties"])
    
    # Load Financial_contracts dataset
    if not client.collections.exists("Financial_contracts"):
        with client_manager.connect_to_client() as client_conn:
            ...
            financial_collection = client_conn.collections.get("Financial_contracts")
            
            with financial_collection.batch.dynamic() as batch:
                for item in financial_dataset:
                    batch.add_object(properties=item["properties"])

    # Check if all required collections exist, skip if not
    if (
...
async def test_aggregate():
    ...
    # Load Ecommerce dataset if not present
    client_manager = ClientManager()
    if not client_manager.client.collections.exists("Ecommerce"):
        import weaviate.classes.config as wvc
        from datasets import load_dataset
