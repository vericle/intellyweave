...
from elysia.tools.text.text import TextResponse, CitedSummarizer, FakeTextResponse
...
class TestMultiBranch:
    ...
    async def test_new_multi_branch(self):
        """
        Test creating a custom multi-branch tree structure.
        This test verifies that branches can be added and the tree can execute
        with a general conversational prompt (no collections required).
        """
        tree = Tree(
            low_memory=False,
            branch_initialisation="empty",
        ...
        # Add FakeTextResponse to base branch as fallback (always available)
        ...
        # Use a simple conversational prompt that doesn't require collections
        async for result in tree.async_run(
