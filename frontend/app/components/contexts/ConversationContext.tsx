...
export const ConversationContext = createContext<{
  ...
  getAllEnabledCollections: (conversationId: string) => string[];
...
export const ConversationProvider = ({
  ...
  const removeConversation = async (conversation_id: string) => {
    // Optimistically remove from state first for better UX
    const wasCurrentConversation = currentConversation === conversation_id;

    // Remove only the specific conversation from state
    setConversations((prevConversations) =>
      prevConversations.filter((c) => c.id !== conversation_id)
    );

    // Remove only the specific conversation preview
    setConversationPreviews((prev) => {
      const newPreviews = { ...prev };
      delete newPreviews[conversation_id];
      return newPreviews;
    });

    // If the deleted conversation was the current one, navigate to the latest remaining conversation
    if (wasCurrentConversation) {
      const remainingPreviews = Object.entries(conversationPreviews).filter(
        ([key]) => key !== conversation_id
      );

      if (remainingPreviews.length > 0) {
        // Navigate to the most recently updated conversation
        const latestConversationId = remainingPreviews.sort(
          ([, a], [, b]) =>
    ...
    // Call the backend deletion API
    try {
      const result = await deleteConversation(id || "", conversation_id);

      if (result.error) {
        console.error(`Failed to delete conversation ${conversation_id}:`, result.error);
        // On error, reload from DB to restore correct state
        await loadConversationsFromDB();
      }
    } catch (error) {
      console.error(`Error deleting conversation ${conversation_id}:`, error);
      // On error, reload from DB to restore correct state
      await loadConversationsFromDB();
    }
  };
  ...
  const getAllEnabledCollections = (conversationId: string) => {
    const conversation = conversations.find((c) => c.id === conversationId);
  ...
  useEffect(() => {
    ...
    // if (process.env.NODE_ENV === "development" && currentConversation) {
