...
function DecisionNode({ data, onNodeClick, selected }: DecisionNodeProps) {
  ...
  // Detect if this is a courthouse node
  const isCourthouseNode =
    data.text?.toLowerCase().includes("courthouse") ||
    data.description?.toLowerCase().includes("courthouse") ||
    data.text?.toLowerCase().includes("debate");
      ...
      <div ref={nodeRef} draggable={false} className="flex gap-2">
        <motion.div
          ...
          className={`
            ...
            ${
              isCourthouseNode && data.choosen
                ? "bg-gradient-to-br from-purple-500/25 via-purple-400/15 to-purple-600/10 text-purple-300 border-2 border-purple-500/50 shadow-lg shadow-purple-500/30 backdrop-blur-sm"
                : isCourthouseNode
                  ? "bg-gradient-to-br from-purple-500/10 to-purple-600/5 text-purple-400/70 border border-purple-500/30 hover:border-purple-500/50"
                  : data.choosen
            ...
            ${isHovered && !data.choosen && isCourthouseNode ? "bg-gradient-to-br from-purple-500/20 to-purple-600/10 text-purple-300/90 border-purple-500/60 shadow-lg shadow-purple-500/20" : ""}
            ${isHovered && !data.choosen && !isCourthouseNode ? "bg-background_alt/90 text-secondary/80 border-secondary/60 shadow-lg" : ""}
            ${isHovered && data.choosen && isCourthouseNode ? "shadow-2xl shadow-purple-500/40" : ""}
            ${isHovered && data.choosen && !isCourthouseNode ? "shadow-2xl shadow-accent/40" : ""}
          `}
          style={{
        ...
        >
          ...
          <div className="flex items-center gap-2 z-10 relative">
            {isCourthouseNode && (
              <span className="text-xl" aria-label="Courthouse">
                ⚖️
              </span>
            )}
            <motion.p
              className={`font-bold text-lg truncate transition-all duration-300 ${
                isCourthouseNode && data.choosen
                  ? "text-purple-300"
                  : isCourthouseNode
                    ? "text-purple-400/70"
